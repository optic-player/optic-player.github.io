...
兼容弹弹Play 弹幕 API，以及 `danmu_api`、`misaka_danmu_server`。对于后两个弹幕 API 服务，播放器会自动判断并兼容其 URL。

- **播放器未内置弹幕源服务器，需自行查找弹幕源并添加。**
- 此弹幕系统为自研的系统，以性能优先为目标，最大限度使用多线程处理。
- 支持自动弹幕匹配和手动弹幕匹配。
- 目前弹幕轨道的数量配置较为保守。同时，如果弹幕太多，造成每秒低于 60 帧时，弹幕调度器会在保证 60 帧的前提下丢弃部分弹幕，以保证性能。
